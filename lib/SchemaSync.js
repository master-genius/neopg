...
class SchemaSync {
  ...
  static async syncIndex(sql, def, schema, curTableName, debug) {
    ...
    let gen_real_index = indname => {
      ...
      if (Array.isArray(indname)) {
        real_indexname = indname.join('_');
      } else if (typeof indname === 'string') {
        if (indname.indexOf(',') >= 0) {
          real_indexname = indname.split(',').map(s=>s.trim()).filter(s=>s).join('_');
        } else {
  ...
  static async autoRemoveIndex(sql, def, schema, tableName, debug) {
      ...
      const makeName = (n) => `${tableName}_${Array.isArray(n)
                        ? n.map(x=>x.trim()).join('_')
                        : n.split(',').map(x=>x.trim()).filter(x=>x).join('_')}_idx`;
